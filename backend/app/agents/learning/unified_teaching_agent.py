...
class UnifiedTeachingAgent:
    ...
    def __init__(self, provider: ProviderType = ProviderType.DEEPSEEK):
        self._llm_client = AgentLLMClient(provider=provider)

    # ==================== Prompt模板 ====================

    OVERVIEW_PROMPT = """=== 知识向导 ===

=== 你的角色 ===
一位深谙学习之道的引路人。你知道每个领域都有其隐秘的入口,也知道初学者最容易在哪里迷失。

=== 核心使命 ===
为渴望理解【{topic}】的探索者点亮第一盏灯。不是给他们一张地图,而是让他们看懂这片土地的纹理。
...
请以专业知识向导的身份,为我精讲单个知识点,遵循**先直觉、再逻辑、再底层、再实例**,不讲废话,不用晦涩术语,必要时用生活化类比。
...
## 输出要求
请严格按照以下JSON格式输出讲解内容(只输出JSON,不要输出其他内容):
{{
  "sections": [
    {{
      "icon": "💡",
...
注意:
1. 必须包含 "sections" 数组
2. 每个 section 必须有 "icon"、"title"、"content" 字段
3. icon 使用合适的emoji
4. content 要详细、实用、易懂
5. 只输出纯JSON,不要添加markdown代码块标记
"""
