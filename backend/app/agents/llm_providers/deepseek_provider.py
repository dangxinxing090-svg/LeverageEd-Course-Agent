...
# 尝试导入openai库（DeepSeek API兼容OpenAI格式）
try:
    from openai import AsyncOpenAI
    HAS_OPENAI = True
except ImportError:
    HAS_OPENAI = False
    logger.warning("未安装openai库，DeepSeekProvider将使用模拟模式")
...
class DeepSeekProvider(LLMProvider):
    ...
    API兼容OpenAI格式，使用AsyncOpenAI客户端
    """
    ...
    async def initialize(self) -> None:
        """初始化DeepSeek客户端"""
        if not HAS_OPENAI:
            logger.warning("openai库未安装，DeepSeekProvider使用模拟模式")
        ...
        try:
            ...
            self._initialized = True
            logger.info(f"DeepSeek Provider已初始化，模型: {self.config.model}")
        except Exception as e:
            logger.error(f"DeepSeek Provider初始化失败: {e}")
            raise
    ...
    async def generate(
        ...
        **kwargs
        ...
        # 转换消息格式
        ds_messages = [msg.to_dict() for msg in messages]
    ...
    def _mock_generate(self, messages: List[LLMMessage]) -> LLMResponse:
        ...
        # 返回模拟响应
        mock_content = f"[DeepSeek模拟响应] 收到消息: {user_message[:50]}..."
